"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.POST = void 0;
var server_1 = require("next/server");
var prisma_1 = require("@/lib/prisma");
var sendTicketEmails_1 = require("@/lib/sendTicketEmails");
var generateTicketImage_1 = require("@/lib/generateTicketImage");
var generateQr_1 = require("@/lib/generateQr");
function generateTickets(orderId, counts) {
    var tickets = [];
    function push(count, category, tier) {
        for (var i = 0; i < count; i++) {
            var code = "" + category[0] + tier[0] + "-" + Math.floor(100000 + Math.random() * 900000);
            tickets.push({
                orderId: orderId,
                category: category,
                tier: tier,
                ticketCode: code,
                qrCode: code
            });
        }
    }
    push(counts.adultLounge, "ADULT", "LOUNGE");
    push(counts.adultStandard, "ADULT", "STANDARD");
    push(counts.childLounge, "CHILD", "LOUNGE");
    push(counts.childStandard, "CHILD", "STANDARD");
    return tickets;
}
function POST(req) {
    return __awaiter(this, void 0, void 0, function () {
        var body, name, email, contactNo, eventId, adultLounge, adultStandard, childLounge, childStandard, totalAmount, event_1, amountInCents, order_1, ticketData, tickets, ticketImages, err_1;
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 8, , 9]);
                    return [4 /*yield*/, req.json()];
                case 1:
                    body = _a.sent();
                    name = body.name, email = body.email, contactNo = body.contactNo, eventId = body.eventId, adultLounge = body.adultLounge, adultStandard = body.adultStandard, childLounge = body.childLounge, childStandard = body.childStandard, totalAmount = body.totalAmount;
                    if (!name || !email || !eventId) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: "Missing required fields" }, { status: 400 })];
                    }
                    return [4 /*yield*/, prisma_1.prisma.event.findUnique({
                            where: { id: eventId }
                        })];
                case 2:
                    event_1 = _a.sent();
                    if (!event_1) {
                        return [2 /*return*/, server_1.NextResponse.json({ error: "Event not found" }, { status: 404 })];
                    }
                    amountInCents = Math.round(Number(totalAmount) * 100);
                    return [4 /*yield*/, prisma_1.prisma.order.create({
                            data: {
                                name: name,
                                email: email,
                                contactNo: contactNo,
                                eventId: eventId,
                                adultLounge: adultLounge,
                                adultStandard: adultStandard,
                                childLounge: childLounge,
                                childStandard: childStandard,
                                totalAmount: amountInCents,
                                paid: true,
                                status: "manual"
                            }
                        })];
                case 3:
                    order_1 = _a.sent();
                    ticketData = generateTickets(order_1.id, {
                        adultLounge: adultLounge,
                        adultStandard: adultStandard,
                        childLounge: childLounge,
                        childStandard: childStandard
                    });
                    return [4 /*yield*/, prisma_1.prisma.ticket.createMany({ data: ticketData })];
                case 4:
                    _a.sent();
                    return [4 /*yield*/, prisma_1.prisma.ticket.findMany({
                            where: { orderId: order_1.id }
                        })];
                case 5:
                    tickets = _a.sent();
                    return [4 /*yield*/, Promise.all(tickets.map(function (t) { return __awaiter(_this, void 0, void 0, function () {
                            var qrBuffer, qrBase64, brandedPng;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0: return [4 /*yield*/, generateQr_1.generateQr(t.qrCode)];
                                    case 1:
                                        qrBuffer = _a.sent();
                                        qrBase64 = qrBuffer.toString("base64");
                                        return [4 /*yield*/, generateTicketImage_1.generateBrandedTicket({
                                                qrPng: qrBase64,
                                                event: event_1.title,
                                                name: order_1.name || "Guest",
                                                date: event_1.date.toString(),
                                                venue: event_1.venue,
                                                category: t.category,
                                                tier: t.tier,
                                                ticketCode: t.ticketCode
                                            })];
                                    case 2:
                                        brandedPng = _a.sent();
                                        return [2 /*return*/, {
                                                category: t.category,
                                                tier: t.tier,
                                                code: t.ticketCode,
                                                image: brandedPng
                                            }];
                                }
                            });
                        }); }))];
                case 6:
                    ticketImages = _a.sent();
                    // Send email
                    return [4 /*yield*/, sendTicketEmails_1.sendTicketEmail({
                            to: email,
                            order: __assign(__assign({}, order_1), { event: event_1,
                                tickets: tickets }),
                            tickets: ticketImages
                        })];
                case 7:
                    // Send email
                    _a.sent();
                    return [2 /*return*/, server_1.NextResponse.json({ order: order_1 })];
                case 8:
                    err_1 = _a.sent();
                    console.error("Admin ticket creation error:", err_1);
                    return [2 /*return*/, server_1.NextResponse.json({ error: "Internal server error", details: err_1.message }, { status: 500 })];
                case 9: return [2 /*return*/];
            }
        });
    });
}
exports.POST = POST;
